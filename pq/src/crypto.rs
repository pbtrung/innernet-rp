...
unsafe extern "C" {
    fn pq_init() -> i32;
    fn pq_kem_keygen(pk: *mut u8, sk: *mut u8, seed: *const u8) -> i32;
    fn pq_kem_enc(ct: *mut u8, ss: *mut u8, pk: *const u8) -> i32;
    fn pq_kem_dec(ss: *mut u8, ct: *const u8, sk: *const u8) -> i32;
    fn pq_x448_public(pk: *mut u8, sk: *const u8) -> i32;
    fn pq_x448(out: *mut u8, pk: *const u8, sk: *const u8) -> i32;
...
pub fn kem_keypair(rng: &mut impl Random) -> Result<([u8; 1568], Secret<3168>)> {
    let seed = random::<64>(rng)?;
    let mut public = [0; 1568];
    let mut secret = Secret::from_bytes([0; 3168]);
    // SAFETY: pointers refer to live nonoverlapping arrays of the ABI's exact sizes.
    invoke(|| unsafe {
        pq_kem_keygen(public.as_mut_ptr(), secret.0.as_mut_ptr(), seed.0.as_ptr())
    })?;
    Ok((public, secret))
}
...
pub fn encapsulate(public: &[u8; 1568]) -> Result<([u8; 1568], Secret<32>)> {
    validate_kem(public)?;
    let mut ciphertext = [0; 1568];
    let mut shared = Secret::from_bytes([0; 32]);
...
pub fn decapsulate(ciphertext: &[u8; 1568], private: &Secret<3168>) -> Result<Secret<32>> {
    let mut shared = Secret::from_bytes([0; 32]);
...
pub fn x448_public(private: &Secret<56>) -> Result<[u8; 56]> {
    let mut public = [0; 56];
    // SAFETY: exact ABI buffer sizes and live references.
    invoke(|| unsafe { pq_x448_public(public.as_mut_ptr(), private.0.as_ptr()) })?;
    Ok(public)
}
pub fn x448(public: &[u8; 56], private: &Secret<56>) -> Result<Secret<56>> {
    let mut shared = Secret::from_bytes([0; 56]);
    // SAFETY: exact ABI buffer sizes and live references.
    invoke(|| unsafe { pq_x448(shared.0.as_mut_ptr(), public.as_ptr(), private.0.as_ptr()) })?;
    if bool::from(shared.0.as_slice().ct_eq(&[0; 56])) {
