...
complete -c innernet -n "__fish_innernet_needs_command" -f -a "export-peer-config" -d 'Refresh a previously `--export-wg-conf`-exported static config\'s peer list, without rotating its keys or requiring the non-innernet device to do anything'
...
complete -c innernet -n "__fish_innernet_using_subcommand help; and not __fish_seen_subcommand_from install show up fetch uninstall down add-peer export-peer-config rename-peer add-cidr rename-cidr delete-cidr list-cidrs disable-peer enable-peer add-association delete-association list-associations set-listen-port override-endpoint override-peer-endpoint completions help" -f -a "export-peer-config" -d 'Refresh a previously `--export-wg-conf`-exported static config\'s peer list, without rotating its keys or requiring the non-innernet device to do anything'
