...
mod tests {
    ...
    fn test_add_peers() {
        ...
        assert_eq!(builder.peers.len(), keypairs.len());
        for (peer, keypair) in builder.peers.iter().zip(&keypairs) {
            assert_eq!(peer.public_key, keypair.public);
        }
    }

    #[test]
    #[ignore = "requires an isolated network namespace, root, and wireguard-go; never run on a production host"]
    fn userspace_interface_add_peers() {
        assert_eq!(unsafe { libc::getuid() }, 0, "root is a precondition");
        let keypairs: Vec<_> = (0..10).map(|_| KeyPair::generate()).collect();
        let mut builder = DeviceUpdate::new();
        for keypair in &keypairs {
            builder = builder.add_peer(PeerConfigBuilder::new(&keypair.public));
