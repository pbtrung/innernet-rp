...
impl DatabasePeer {
        ...
        let invite_expires = invite_expires
            .map(|t| {
                let seconds = t
                    ...
                    .as_secs();
                i64::try_from(seconds).map_err(|_| ServerError::InvalidQuery)
            })
            .transpose()?;
    ...
    fn from_row(row: &rusqlite::Row) -> Result<Self, rusqlite::Error> {
        ...
        let invite_expires = row
            .get::<_, Option<i64>>(9)?
            .map(|unixtime| {
                let seconds = u64::try_from(unixtime)
                    .map_err(|_| rusqlite::Error::IntegralValueOutOfRange(9, unixtime))?;
                SystemTime::UNIX_EPOCH
                    .checked_add(Duration::from_secs(seconds))
                    .ok_or(rusqlite::Error::IntegralValueOutOfRange(9, unixtime))
            })
            .transpose()?;
    ...
    pub fn delete_expired_invites(conn: &Connection) -> Result<usize, ServerError> {
        ...
        let deleted = conn.execute(
            ...
            params![i64::try_from(unix_now.as_secs()).map_err(|_| ServerError::InvalidQuery)?],
        )?;
